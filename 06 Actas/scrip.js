...
// NUEVO BOTÓN: Copiar el recibo actual en JSON
document.getElementById('botonCopiarJson').addEventListener('click', function() {
    if (reciboActual) {
        // Convirtiendo el recibo actual a JSON con formato
        const jsonRecibo = JSON.stringify(reciboActual, null, 2);
        navigator.clipboard.writeText(jsonRecibo)
            .then(() => {
                alert('Recibo copiado al portapapeles en formato JSON.');
            })
            .catch(err => {
                console.error('Error al copiar el recibo:', err);
            });
    } else {
        alert('No hay un recibo para copiar.');
    }
});
