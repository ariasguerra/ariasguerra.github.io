document.addEventListener('DOMContentLoaded', () => {
    ...
    // Nuevo elemento para las sugerencias predictivas
    let suggestionsDiv;
    ...
    // Evento para el input de búsqueda (buscador predictivo)
    searchInput.addEventListener('input', function() {
        clearTimeout(typingTimer);
        if (searchInput.value) {
            typingTimer = setTimeout(showPredictions, doneTypingInterval);
        } else {
            suggestionsDiv.style.display = 'none';
        }
    });

    // Evento para ocultar las sugerencias cuando se pierde el foco
    searchInput.addEventListener('blur', function(e) {
        // Pequeño retraso para permitir que se capture el clic en una sugerencia
        setTimeout(() => {
            suggestionsDiv.style.display = 'none';
        }, 200);
    });
    ...
    searchForm.addEventListener('submit', (e) => {
        ...
        if (searchTerm) {
            ...
            displayCurrentContact();
            updateNavigation();
            suggestionsDiv.style.display = 'none'; // Ocultar sugerencias después de la búsqueda
        } else {
    ...
    // Función para mostrar predicciones basadas en la entrada actual
    function showPredictions() {
        ...
        const matches = searchContacts(searchTerm).slice(0, 5); // Limitar a 5 sugerencias
        ...
        if (matches.length > 0) {
            ...
            matches.forEach(contact => {
                ...
                suggestion.innerHTML = `
                    <span class="suggestion-grado">${gradoCompleto}</span>
                    <span class="suggestion-nombre">${contact.NOMBRES || ''} ${contact.APELLIDOS || ''}</span>
                `;
