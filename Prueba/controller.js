...
const AppController = (function(model, ui, utils) {
    ...
    function handleFileInput(e) {
        ...
        if (isExcel) {
            ...
            ui.showMessage("Procesando archivo Excel...", 10000);
            
            model.loadFromExcelFile(file)
                .then(contacts => {
                    if (contacts && contacts.length > 0) {
                        ...
                        model.setCurrentIndex(0);
                        updateCurrentContactView();
                        ui.showMessage(`${contacts.length} contactos cargados desde Excel`, 3000);
                    }
                })
                .catch(error => {
                    console.error('Error al cargar Excel:', error);
                    ui.showMessage("Error al procesar el archivo Excel: " + error, 5000);
                });
        } else if (isJson) {
            // Mantener el código existente para archivos JSON
            ...
            ui.showMessage("Formato de archivo no soportado. Use .xlsx, .xls o .json", 3000);
        }
    }
